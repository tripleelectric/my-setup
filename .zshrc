...
ytmedium() {
    ...
    if [[ "$TARGET" == "vault" ]]; then
        ...
        DEST="/Volumes/$VOLUME_NAME"
    else
        DEST="$PWD"
    fi

    echo "⬇️  Downloading into: $DEST"
    yt-dlp -f "bv[height<=720]+ba/best[height<=720]" -o "$DEST/%(title)s.%(ext)s" "$VIDEO_URL"
}
...
ytaudio() {
    ...
    echo "Downloading audio into: $DEST"
    yt-dlp -x --audio-format mp3 -o "$DEST/%(title)s.%(ext)s" "$VIDEO_URL"
}

# Download YouTube playlist (Video + Audio, up to 720)
ytplvid() {
  yt-dlp -f "bv*[height<=720]+ba/best[height<=720]" \
    -o "%(playlist_title)s/%(playlist_index)03d - %(title)s.%(ext)s" \
    "$1"
}

# Download YouTube playlist (Audio Only, best quality MP3)
ytplaud() {
  yt-dlp -f "bestaudio" \
...
ytdl_list() {
  ...
  for arg in "$@"; do
    case "$arg" in
      ...
      *.txt)
        ...
        ;;
      *)
        echo "⚠️ Ignored unknown argument: $arg"
        ;;
    esac
  done
  ...
  case "$res" in
    low)
      format="bv[height<=360]+ba/best[height<=360]"
      ;;
    medium)
  ...
  echo "📥 Downloading from '$list_file' at '$res' quality..."
  yt-dlp -a "$list_file" -f "$format" -N 4 -o "%(title)s.%(ext)s"
}
yt_interactive() {
  ...
  select input_type in "Single Video URL" "Playlist URL" "Links File"; do
    case $REPLY in
      1) read -p "Enter video URL: " input_value; break ;;
      2) read -p "Enter playlist URL: " input_value; break ;;
      3) read -p "Enter file name with links: " input_value; break ;;
      *) echo "Invalid choice";;
    esac
  done
  ...
  select download_type in "Video+Audio" "Audio Only (MP3)"; do
    case $REPLY in
      1) format_type="video"; break ;;
      2) format_type="audio"; break ;;
      *) echo "Invalid choice";;
    esac
  done

  # Ask for resolution if video
  if [[ "$format_type" == "video" ]]; then
    echo "Choose resolution:"
    select res in "Low (360p)" "Medium (720p)" "High (1080p or best)"; do
      case $REPLY in
        1) format="bv[height<=360]+ba/best[height<=360]"; break ;;
        2) format="bv[height<=720]+ba/best[height<=720]"; break ;;
        3) format="bv*+ba/best"; break ;;
        *) echo "Invalid choice";;
      esac
    done
  else
    # Audio only
    format="bestaudio"
    read -p "Convert audio to (mp3/opus/m4a) [default mp3]: " audio_fmt
    audio_fmt="${audio_fmt:-mp3}"
  fi

  # Output template
  read -p "Enter output folder (leave empty for current folder): " out_folder
  out_folder="${out_folder:-.}"

  if [[ "$format_type" == "audio" ]]; then
    yt-dlp -a "$input_value" -f "$format" -x --audio-format "$audio_fmt" -o "$out_folder/%(title)s.%(ext)s"
  else
    yt-dlp -a "$input_value" -f "$format" -N 4 -o "$out_folder/%(title)s.%(ext)s"
  fi
